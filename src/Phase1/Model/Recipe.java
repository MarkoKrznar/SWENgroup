 ...
 * if (entry.getFood().getName().equals(food.getName())) { // checks if the
 * ingredient alr exists
 * entry.setServings(entry.getServings() + servings); // if its a duplicate,
 * increase serving size
 * return;
 * }
 * }
 * ingredients.add(new IngredientEntry(food, servings)); // if not dup, add new
 * ingredient entry
 * }
 * 
 * public void removeIngredient(String foodName) { // remove ingredient
 * for (int i = 0; i < ingredients.size(); i++) {
 * if (ingredients.get(i).getFood().getName().equals(foodName)) { // if its the
 * food we want to remove
 ...
 * double total = 0;
 * for (IngredientEntry entry : ingredients) {
 * total += entry.getFood().getCalories() * entry.getServings(); // cals are per
 * serving so cals * srving is total amount
 * }
 * return total;
 * }
 * 
